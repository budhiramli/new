...
class Clearing extends CI_Controller {
    ...
    function index()
    {
        ...
        $content .= $this->twiggy->template('list/clearing')->render();
        // end        
        ...
        $script_page = $this->twiggy->template('script/clearing')->render();         
    ...
    function form($id='')
    {
        if (!empty($id)){
            $this->load->model('modelclearing');
            $data = $this->modelclearing->getdataid($id);
    ...
    function save()
    {
        ...
        $valid = $this->modelclearing->save($params);
